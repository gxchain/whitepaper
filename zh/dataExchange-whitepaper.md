...
#### 3.⾃由市场（Free Market）和联盟市场（Alliance Market）
...
公信链采取和graphene⼀样的witness和light-wallet分离的机制，需要同步全 部账本的节点可以运⾏witness（full node），⽽⼤部分⽤户则只需要运⾏⼏⼗M的lightwallet即可，light-wallet可以链接到任何⼀个witness，⽽公信链在light-wallet上做了更近 ⼀步的开发（参考6.1说明），开发了⼤量JSON-RPC，商户购买数据和数据源接⼊以及联 盟市场都只需要对light-wallet做接⼝开发即可，不再强制依赖witness。当然如果有些商户 GONGXINBAO BLOCKCHAIN 公信宝区块链 BD@GXB.IO 16 杭州存信数据科技有限公司 希望同步full node，则可以在商户⾃⼰的服务器上安装witness，让right-wallet链接上这个 节点即可。
...
1、进鱼花了46元，鱼给了别人 而收到的是假钱，没有赚到卖鱼钱；    此时亏 -46

2、不仅没赚到钱，还向邻居借了100元，找给买鱼人30元，手里剩下70元；  

3、当邻居发现100是假钱，赔邻居的100，拿出第二步剩下的70，还需要自己掏腰包贴30元 ，此时亏 - 30
