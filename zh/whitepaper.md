...
近几年来，随着互联网的高速发展和信息技术的普及应用，行业应用所产生的数据呈爆炸性增长，在快速发展的同时也带来了数据的垄断和信息孤岛，大数据交换背后所带来的巨大价值也充满了无限想象。
...
公信链（GXChain）是一条公有链，是公信宝数据交易所的底层区块链，不仅支撑着公信宝数据交易所高频的数据交易，还支持第三方开发应用，在公信链上开发不仅可以实现高性能链上应用，还可以获得公信宝全行业多维度数据的支持，可以做出非常落地于民生的有价值应用。
